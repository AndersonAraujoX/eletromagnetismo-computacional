program tarefaA
  ...
  enddo
  do i=-N+1,N-1
    do j=-N+1,N-1!depositando os dados VxR 
      do k=-N+1,N-1
        if(j==0 .and. k==0 .and. i>0)then
          write(13,*)i*1.0/N,volt(i,j,k)
        endif
        if(i==j .and. k==0 .and. i>0 .and. j>0)then
          write(14,*)sqrt((i*1.0/N)*(i*1.0/N)+(j*1.0/N)*(j*1.0/N)),volt(i,j,k)
        endif
        if(i==j .and. k==i .and. k>0 .and. i>0 .and. j>0)then
          write(15,*)sqrt((i*1.0/N)*(i*1.0/N)+(j*1.0/N)*(j*1.0/N)+(k*1.0/N)*(k*1.0/N)),volt(i,j,k)
        endif
      enddo
    enddo
  enddo
